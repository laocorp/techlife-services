...
export default function AddEventForm({ orderId }: { orderId: string }) {
    ...
    return (
        <div className="bg-card border border-border rounded-lg p-4 shadow-sm space-y-4">
            <h3 className="text-sm font-medium text-foreground">Agregar Nota o Evidencia</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {file && (
                    <div className="flex items-center gap-2 text-sm bg-muted p-2 rounded border border-border text-foreground">
                        <Paperclip className="h-4 w-4 text-purple-600 dark:text-purple-400" />
                        <span className="truncate max-w-[200px]">{file.name}</span>
                        <button type="button" onClick={() => setFile(null)} className="ml-auto hover:bg-muted-foreground/20 rounded-full p-1">
